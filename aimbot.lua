...
pcall(function()
    if _G.stepped then
        ...
        _G.stepped = nil
        _G.input = nil
        print("[DEBUG]: Resetting connections...")
    end

    if not reset then
        print("[DEBUG]: No reset required. Loading...")
    end
end)
...
local function changePower(value)
    pcall(function()
        ...
        print("[DEBUG]: Power changed to", value)
    end)
end
...
local function removed(child)
    if child.Name == "Basketball" then
        hasBall = false
        print("[DEBUG]: Basketball removed")
    end
end
